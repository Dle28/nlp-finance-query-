...
| Active-learning control plane V1 | Private Kaggle GPU shadow cycle completed; no promotion | The V5 frozen cycle partitions 1,012 questions into 184 proof-policy clusters and selects 64 active-learning plus 32 independently probability-sampled audit packets. The competition model graph is open-source-only and strictly below 14.7B: exact-revision/weight-SHA-pinned Qwen3-8B proposer plus Mistral-Nemo-12B critic. The clean private Kaggle run at source commit `362b0ed6…` processed all 64 packets: Qwen produced 15 schema-valid proposals and 49 non-proposals; the blind Mistral critic produced 0 schema-valid proposals and 64 non-proposals. Reconciliation therefore emitted 0 agreements and 64 escalations. The downloaded 59,576-byte receipt archive passed ZIP integrity and SHA-256 `fbb79bdc…`. ChatGPT remains outside training/inference/ensembles and may only perform bounded `chatgpt_verified` human-equivalent escalation review. Current training records and provisional policies remain 0; release remains blocked. |
