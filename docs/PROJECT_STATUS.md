...
| Active-learning control plane V1 | GPU job prepared and replay-verified; inference not run | The V5 frozen cycle partitions 1,012 questions into 184 proof-policy clusters and selects 64 active-learning plus 32 independently probability-sampled audit packets. The competition model graph is open-source-only and strictly below 14.7B: exact-revision/weight-SHA-pinned Qwen3-8B proposer plus Mistral-Nemo-12B critic. The 64 blind model packets now include their typed, numeric-free closure projection rather than only an opaque hash; model-job manifest `33a5e1fa…` replays deterministically and a portable Kaggle bundle is prepared. Local CUDA and Kaggle CLI credentials are unavailable, so GPU inference is truthfully `not_run`. ChatGPT remains outside training/inference/ensembles and may only perform bounded `chatgpt_verified` human-equivalent escalation review. A single model agreement stays non-materializable; current training records and provisional policies are both 0; release remains blocked. |
