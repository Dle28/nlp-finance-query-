"""Validate the temporal-header receipt before opening the unseen source split."""
...
def validate_temporal_pre_unseen(receipt_path: Path, repo_root: Path) -> dict[str, Any]:
    ...
    for name, descriptor in inputs.items():
        if not isinstance(descriptor, Mapping):
            errors.append(f"invalid descriptor: {name}")
            continue
    ...
    candidate = (development.get("arms") or {}).get("B_A_plus_source_header_backoff") or {}
    outcomes = development.get("candidate_outcome_counts") or {}
    observed = {
        "newly_correct": int(outcomes.get("IMPROVED", 0)),
        "accuracy_on_candidate_predictions": candidate.get("accuracy_on_predictions"),
        "false_confident_candidate": int(candidate.get("false_confident_count", 0)),
        "regressed_question_only_predictions": int(outcomes.get("REGRESSED", 0)),
    }
    ...
    passes = (
        ...
        and observed["accuracy_on_candidate_predictions"]
        == thresholds.get("minimum_accuracy_on_candidate_predictions")
        and observed["false_confident_candidate"]
        <= thresholds.get("maximum_false_confident_candidate", 0)
        and observed["regressed_question_only_predictions"]
        <= thresholds.get("maximum_regressed_question_only_predictions", 0)
    )
    if not passes:
    ...
    required = {
        ...
        "minimum_accuracy_on_candidate_predictions",
        "maximum_false_confident_candidate",
        "maximum_regressed_question_only_predictions",
    ...
    if not (
        ...
        errors.append("receipt source contract is unsafe")
    return {
        "protocol": "temporal_header_semantics_pre_unseen_validation_v1",
