"""Value-blind phrase and header recheck for explicit-ticker direct lookups.

This module is deliberately a narrow source-navigation experiment.  It uses
one documented phrase rule per previously planner-blocked direct lookup, then
reapplies V2/V3 provenance, typed table function, scope, period, unit and row
margin gates.  It cannot bind evidence, execute a value, or produce a
...
from finance_query.research.staged_formula_metric_phrase_probe import (
    ...
    _compact,
    _contains_forbidden,
    _explicit_year_end_header_status,
    _semantic_rows,
)
...
PROTOCOL = "vifinqa_explicit_ticker_semantic_source_probe_v1"
CONTRACT = {
    "research_only": True,
...
def _rules(path: Path) -> dict[tuple[int, str], dict[str, Any]]:
    ...
    if payload.get("protocol") != PROTOCOL or int(payload.get("schema_version") or 0) != 1:
        raise ValueError("unexpected explicit ticker semantic probe config")
    rules: dict[tuple[int, str], dict[str, Any]] = {}
    for raw in payload.get("rules") or []:
        if not isinstance(raw, Mapping):
            raise ValueError("semantic rule must be an object")
        question_id, operand_id = int(raw.get("question_id") or 0), str(raw.get("operand_id") or "")
        required = tuple(str(value) for value in raw.get("required_compact_fragments") or [])
        forbidden = tuple(str(value) for value in raw.get("forbidden_compact_fragments") or [])
        functions = tuple(str(value) for value in raw.get("allowed_table_functions") or [])
        ...
        if (
            ...
            or not rule_id
            or not required
            or any(not value or _compact(value) != value for value in (*required, *forbidden))
            or any(not value for value in functions)
            or key in rules
        ):
            raise ValueError("semantic rule is incomplete or duplicated")
        rules[key] = {
            "rule_id": rule_id,
            "required": required,
            "forbidden": forbidden,
            ...
            "allowed_table_functions": functions,
        }
    if not rules:
        raise ValueError("semantic probe has no rules")
    return rules


def _effective_operand(operand: Mapping[str, Any], rule: Mapping[str, Any]) -> dict[str, Any]:
    result = deepcopy(dict(operand))
    functions = list(rule.get("allowed_table_functions") or [])
    if functions:
        result["allowed_table_functions"] = functions
    return result


def build_explicit_ticker_semantic_source_probe(
    *,
    config_path: Path,
    plans_path: Path,
    targets_path: Path,
    ...
    target_ids = {int(row["question_id"]) for row in _read_jsonl(targets_path)}
    if set(plans) != set(range(1, expected_question_count + 1)) or not target_ids:
        raise ValueError("plans or targets have invalid coverage")
    target_keys: set[tuple[int, str]] = set()
    for question_id in target_ids:
        plan = plans.get(question_id) or {}
        for operand in plan.get("operands") or []:
            key = (question_id, str(operand.get("operand_id") or ""))
            if key not in rules:
                raise ValueError("target operand has no semantic rule")
            target_keys.add(key)
    if target_keys != set(rules):
        raise ValueError("semantic config must cover exactly the target operands")
    ...
    descriptor = (candidate_manifest.get("outputs") or {}).get(table_candidate_path.name) or {}
    if sha256_file(table_candidate_path) != descriptor.get("sha256"):
    ...
    if not requested_uids or "" in requested_uids:
        raise ValueError("semantic target has no candidate tables")
    assets = {
    ...
    if requested_uids - set(assets):
        raise ValueError("candidate table is absent from immutable assets")
    current_tables = {
        str(row.get("internal_table_uid") or ""): row
    ...
    for question_id, operand_id in sorted(target_keys):
        ...
        original_operand = next(row for row in plan.get("operands") or [] if str(row.get("operand_id") or "") == operand_id)
        ...
        literal = (
            {"status": "OPERAND_PLAN_NOT_SINGLE_ENTITY_AND_YEAR", "strict_source_candidate_count": 0, "navigation_candidates": [], "first_stage_failed_gate_counts": {}, "second_stage_failed_gate_counts": {}}
            if subplan is None
            else _strict_status(source_rows=semantic_rows, plan=subplan, tables=tables, contexts=contexts, minimum_row_jaccard=minimum_row_jaccard, minimum_row_margin=minimum_row_margin)
        )
        current = _current_period_status(source_rows=semantic_rows, operand=operand, tables=tables, contexts=contexts, minimum_row_jaccard=minimum_row_jaccard, minimum_row_margin=minimum_row_margin)
        explicit_date = _explicit_year_end_header_status(source_rows=semantic_rows, operand=operand, tables=tables, contexts=contexts, minimum_row_jaccard=minimum_row_jaccard, minimum_row_margin=minimum_row_margin)
        record = {
            "schema_version": 1,
            "protocol": PROTOCOL,
        ...
        if _contains_forbidden(record):
            raise ValueError("semantic source probe leaked a forbidden field")
        audit.append(record)
    summary = {
        "schema_version": 1,
        "protocol": PROTOCOL,
    ...
    try:
        audit_path = temporary / "explicit_ticker_semantic_source_probe_v1.jsonl"
        summary_path = temporary / "explicit_ticker_semantic_source_probe_summary_v1.json"
...
def validate_explicit_ticker_semantic_source_probe(artifact_dir: Path, *, expected_question_count: int = 1012) -> dict[str, Any]:
    manifest = _read_json(artifact_dir / "manifest.json")
    if manifest.get("protocol") != PROTOCOL or manifest.get("source_contract") != CONTRACT:
        raise ValueError("unexpected explicit ticker semantic probe contract")
    for group, relative in (("inputs", False), ("outputs", True)):
        for descriptor in (manifest.get(group) or {}).values():
            ...
            if not path.is_file() or sha256_file(path) != descriptor.get("sha256"):
                raise ValueError("explicit ticker semantic probe hash mismatch")
    audit = _read_jsonl(artifact_dir / "explicit_ticker_semantic_source_probe_v1.jsonl")
    if not audit or not {int(row["question_id"]) for row in audit} <= set(range(1, expected_question_count + 1)):
        raise ValueError("explicit ticker semantic probe question coverage is invalid")
    if any(_contains_forbidden(row) or row.get("raw_numeric_values_included") is not False or row.get("source_contract") != CONTRACT for row in audit):
        raise ValueError("explicit ticker semantic probe lost its boundary")
    summary = _read_json(artifact_dir / "explicit_ticker_semantic_source_probe_summary_v1.json")
    counts = dict(sorted(Counter(row["combined_status"] for row in audit).items()))
    if summary.get("target_operand_count") != len(audit) or summary.get("combined_status_counts") != counts:
        raise ValueError("explicit ticker semantic probe summary mismatch")
