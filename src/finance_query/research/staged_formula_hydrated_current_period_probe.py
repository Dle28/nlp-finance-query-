"""Recheck current-period headers after in-memory V2/V3 sidecar hydration.

The probe closes a blind spot in the ordinary current-period check: a missing
sidecar can hide a separate or consolidated candidate and make another source
look unique.  It reconstructs only immutable OCR slices in memory, compares
the two coverage views, and stays entirely outside E2E authorization.
"""
...
PROTOCOL = "vifinqa_staged_formula_hydrated_current_period_probe_v1"
CONTRACT = {
    "research_only": True,
...
def build_staged_formula_hydrated_current_period_probe(
    *,
    plans_path: Path,
    targets_path: Path,
    ...
    if any(plans[question_id].get("decomposition_status") != "typed_non_executable" for question_id in targets):
        raise ValueError("hydrated current-period targets must be typed staged plans")
    ...
    for question_id in sorted(targets):
        for operand in plans[question_id].get("operands") or []:
            ...
            baseline = _current_period_status(
            ...
            hydrated = _current_period_status(
                source_rows=source_rows,
                operand=operand,
            ...
            record = {
                ...
                "hydrated": hydrated,
                "improved_to_unique": (
                    baseline["operand_status"] != "UNIQUE_CURRENT_PERIOD_RECHECK_CANDIDATE"
                    and hydrated["operand_status"] == "UNIQUE_CURRENT_PERIOD_RECHECK_CANDIDATE"
                ),
                "raw_numeric_values_included": False,
                "source_contract": dict(CONTRACT),
            }
            if _contains_forbidden(record):
                raise ValueError("hydrated current-period probe leaked a forbidden field")
            audit.append(record)
    summary = {
        "schema_version": 1,
        "protocol": PROTOCOL,
        ...
        "baseline_status_counts": dict(sorted(Counter(row["baseline"]["operand_status"] for row in audit).items())),
        "hydrated_status_counts": dict(sorted(Counter(row["hydrated"]["operand_status"] for row in audit).items())),
    ...
    try:
        audit_path = temporary / "staged_formula_hydrated_current_period_operand_probe_v1.jsonl"
        summary_path = temporary / "staged_formula_hydrated_current_period_probe_summary_v1.json"
...
def validate_staged_formula_hydrated_current_period_probe(
    artifact_dir: Path,
    *,
    expected_question_count: int = 1012,
) -> dict[str, Any]:
    ...
    if manifest.get("protocol") != PROTOCOL or manifest.get("source_contract") != CONTRACT:
        raise ValueError("unexpected hydrated current-period probe contract")
    for group, relative in (("inputs", False), ("outputs", True)):
        for descriptor in (manifest.get(group) or {}).values():
            ...
            if not path.is_file() or sha256_file(path) != descriptor.get("sha256"):
                raise ValueError("hydrated current-period probe hash mismatch")
    ...
    if not targets <= set(range(1, expected_question_count + 1)):
        raise ValueError("hydrated current-period targets are outside the expected population")
    audit = _read_jsonl(artifact_dir / "staged_formula_hydrated_current_period_operand_probe_v1.jsonl")
    if {int(row["question_id"]) for row in audit} != targets:
        raise ValueError("hydrated current-period target coverage mismatch")
    if any(
        ...
        raise ValueError("hydrated current-period probe lost its non-authorizing boundary")
    summary = _read_json(artifact_dir / "staged_formula_hydrated_current_period_probe_summary_v1.json")
    baseline_counts = dict(sorted(Counter(row["baseline"]["operand_status"] for row in audit).items()))
    hydrated_counts = dict(sorted(Counter(row["hydrated"]["operand_status"] for row in audit).items()))
    if (
        ...
        raise ValueError("hydrated current-period probe summary mismatch")
    return {
        "status": "PASS",
        "target_question_count": len(targets),
        "operand_count": len(audit),
