...
This module prepares numeric-free Qwen proposer packets and validates its JSON.
Every valid proposal is still provisional and is routed to independent human
review; no output is answer-, training-, promotion-, or submission-eligible.
"""
...
MODEL_ROLES = ("open_source_model_proposer",)
...
def _routes_by_role(policy_path: Path) -> dict[str, dict[str, Any]]:
    ...
    for route in routes.values():
        ...
        if role not in MODEL_ROLES or role in by_role:
            raise ValueError("model policy must define exactly one proposer route")
    ...
    if set(by_role) != set(MODEL_ROLES):
        raise ValueError("model policy lacks the sole proposer route")
    return by_role
...
def build_model_job(
    *,
    ...
    """Build immutable Qwen proposer requests for active-learning items."""
    ...
    try:
        ...
        request_paths = {MODEL_ROLES[0]: staging / "qwen3_8b_proposer_requests_v1.jsonl"}
...
def render_prompt(request: Mapping[str, Any]) -> str:
    ...
    if role != MODEL_ROLES[0]:
        raise ValueError("only the open-source proposer role is supported")
    role_instruction = "Đề xuất một rule có thể tái kiểm tra cho cluster."
    proposal_shape = {
        "schema_version": 1,
        "protocol": VALIDATED_RESPONSE_PROTOCOL,
...
def reconcile_validated_responses(
    *,
    ...
    if set(packets) != set(proposer):
        raise ValueError("proposer output does not cover the same review packets")
    ...
    for review_id in sorted(packets, key=lambda value: int(packets[value]["question_id"])):
        ...
        response = proposer[review_id]
        _validate_reconciliation_input(response, packet=packet, expected_role=MODEL_ROLES[0])
        if response.get("validation_status") == "VALID_PROPOSAL":
            payload = {
                ...
                "proposer_validated_response_id": response["validated_response_id"],
                "reconciliation_status": "QWEN_PROPOSAL_PENDING_INDEPENDENT_REVIEW",
                "required_reviewer_types": ["human_adjudicator", "chatgpt_human_equivalent_reviewer"],
            ...
            if response.get("validation_status") == "INVALID_MODEL_RESPONSE":
                reason = "INVALID_MODEL_RESPONSE"
            elif response.get("validation_status") == "VALID_ABSTENTION":
                reason = "MODEL_ABSTENTION"
            else:
                reason = "UNEXPECTED_MODEL_VALIDATION_STATE"
            payload = {
                "schema_version": 1,
                "protocol": RECONCILIATION_PROTOCOL,
                "question_id": packet["question_id"],
