...
OPERATOR_REGISTRY: dict[str, OperatorContract] = {
    ...
    # Identity selectors are grounded operators, not pure arithmetic.  The
    # caller must supply one exact binding per period; execution returns the
    # unique winning report year and blocks ties or mixed entities.
    "arg_extreme_period": OperatorContract("arg_extreme_period", 2, None, "same", "period"),
...
def execute_ast(ast: Mapping[str, Any], values: Mapping[str, Any]) -> Any:
    ...
    if op == "arg_extreme_period":
        direction = str(ast.get("direction") or "")
        if direction not in {"min", "max"}:
            raise ValueError("arg_extreme_period requires min or max direction")
        if len(args) < 2:
            raise ValueError("arg_extreme_period requires at least two candidates")
        candidates: list[tuple[int, Decimal]] = []
        for argument in args:
            if not isinstance(argument, Mapping):
                raise ValueError("arg_extreme_period requires grounded period candidates")
            period = argument.get("period")
            if isinstance(period, bool) or not isinstance(period, int):
                raise ValueError("arg_extreme_period candidate lacks an integer period")
            candidates.append((period, _as_decimal(argument.get("value"))))
        if len({period for period, _value in candidates}) != len(candidates):
            raise ValueError("arg_extreme_period requires unique periods")
        extreme = (
            max(value for _period, value in candidates)
            if direction == "max"
            else min(value for _period, value in candidates)
        )
        winners = [period for period, value in candidates if value == extreme]
        if len(winners) != 1:
            raise ValueError("arg_extreme_period has no unique winner")
        return Decimal(winners[0])
    if op == "cagr":
        if len(args) != 3:
...
def execute_grounded_ast_shadow(
    ...
    *,
    ...
    for name in sorted(referenced & set(grounded_inputs)):
        ...
        if not binding_errors:
            if root_op == "arg_extreme_period":
                if not isinstance(value, Decimal) or len(bindings) != 1:
                    binding_errors.append("period_selector_requires_one_exact_binding")
                else:
                    period = bindings[0].get("report_year")
                    if isinstance(period, bool) or not isinstance(period, int):
                        binding_errors.append("period_selector_requires_integer_report_year")
                    else:
                        resolved[name] = {"period": period, "value": value}
            else:
                resolved[name] = value
    ...
    if root_op == "arg_extreme_period":
        entities = {str(item.get("ticker")) for item in all_bindings if item.get("ticker")}
        if len(entities) != 1:
            errors.append("period_selector_entity_mismatch")
        periods = [
            item["report_year"]
            for item in all_bindings
            if isinstance(item.get("report_year"), int)
            and not isinstance(item.get("report_year"), bool)
        ]
        if len(periods) != len(all_bindings):
            errors.append("period_selector_invalid_period")
        if len(periods) != len(set(periods)):
            errors.append("period_selector_duplicate_period")
    ...
    return {
        ...
        "result_period": (
            int(result)
            if result is not None and contract and contract.output_kind == "period"
            else None
        ),
...
def execute_typed_plan_shadow(
    ...
    *,
    ...
    """Bind a complete typed plan to exact inputs before shadow execution.

    The numeric executor validates each source cell.  This wrapper additionally
    proves that every cell belongs to the entity, period and scope declared by
    its typed operand.  It never repairs a plan or chooses a replacement cell.
    """
    errors: list[str] = []
    if plan.get("protocol") != TYPED_OPERAND_PLAN_PROTOCOL:
        errors.append("typed_plan_protocol_mismatch")
    if plan.get("decomposition_status") != "complete":
        errors.append("typed_plan_not_complete")
    ast = plan.get("operation_ast")
    if not isinstance(ast, Mapping):
        errors.append("typed_plan_operation_ast_missing")
        ast = {"op": "abstain", "args": []}
    operands = plan.get("operands")
    if not isinstance(operands, list) or not operands:
        errors.append("typed_plan_operands_missing")
        operands = []
    fingerprint_payload = {
        "effective_family": plan.get("effective_family"),
        "status": plan.get("decomposition_status"),
    ...
    if plan.get("plan_fingerprint") != expected_fingerprint:
        errors.append("typed_plan_fingerprint_mismatch")
    by_id: dict[str, Mapping[str, Any]] = {}
    for operand in operands:
        if not isinstance(operand, Mapping):
            errors.append("typed_plan_operand_invalid")
            continue
        operand_id = str(operand.get("operand_id") or "")
        if not operand_id or operand_id in by_id:
            errors.append("typed_plan_operand_id_invalid")
            continue
        by_id[operand_id] = operand
    referenced = _referenced_inputs(ast)
    ...
    for operand_id, operand in by_id.items():
        ...
        if not isinstance(record, Mapping):
            continue
        expected_entity = str(operand.get("entity") or "").casefold()
        if not expected_entity or str(record.get("ticker") or "").casefold() != expected_entity:
            errors.append(f"{operand_id}:typed_entity_mismatch")
        expected_years = operand.get("years")
        observed_year = record.get("report_year")
        if (
            not isinstance(expected_years, list)
            or len(expected_years) != 1
            or isinstance(expected_years[0], bool)
            or not isinstance(expected_years[0], int)
            or isinstance(observed_year, bool)
            or not isinstance(observed_year, int)
            or observed_year != expected_years[0]
        ):
            errors.append(f"{operand_id}:typed_period_mismatch")
        expected_scope = str(operand.get("scope") or "")
        if expected_scope and str(record.get("scope") or "").casefold() != expected_scope.casefold():
            errors.append(f"{operand_id}:typed_scope_mismatch")
    if errors:
        return {
            "protocol": TYPED_PLAN_GROUNDED_PROTOCOL,
            "status": "shadow_blocked",
            "operator": str(ast.get("op") or ""),
            "result_value": None,
            "result_period": None,
            "output_unit": None,
            "reason_codes": sorted(set(errors)),
    ...
    return {
        **result,
        "protocol": TYPED_PLAN_GROUNDED_PROTOCOL,
        "grounded_operator_protocol": result["protocol"],
