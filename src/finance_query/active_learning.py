...
def _decision_registry(
    ...
    for review_id, item_decisions in decisions_by_review.items():
        ...
        if proposer is None or critic is None or adjudicator is None:
            continue
        policies_match = (
            proposer.get("verdict") == critic.get("verdict") == adjudicator.get("verdict")
            and isinstance(proposer.get("approved_policy"), Mapping)
            and canonical_sha256(proposer.get("approved_policy")) == canonical_sha256(critic.get("approved_policy"))
    ...
    for cluster in clusters:
        ...
        for item_values in by_item.values():
            ...
            same_policy = (
                isinstance(proposer.get("approved_policy"), Mapping)
                and canonical_sha256(proposer.get("approved_policy")) == canonical_sha256(critic.get("approved_policy"))
            )
            if same_verdict and same_policy:
