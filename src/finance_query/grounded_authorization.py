...
from .semantic_binding_corrections import (
    effective_approval_from_correction,
    load_semantic_binding_corrections,
)
from .navigation_binding_promotions import (
    effective_approval_from_navigation_promotion,
    load_navigation_binding_promotions,
)
from .cross_entity_binding_promotions import (
    effective_approvals_from_cross_entity_promotion,
...
def materialize_authorization_replay(
    *,
    ...
    semantic_approval_bindings: Path | None = None,
    ...
    navigation_promotion_base_bindings: Path | None = None,
    ...
    cross_entity_promotion_base_bindings: Path | None = None,
    ...
    if any(path is not None for path in correction_paths) != all(path is not None for path in correction_paths):
        raise GroundedAuthorizationError("semantic correction inputs must be supplied together")
    promotion_paths = (
    ...
    if any(path is not None for path in promotion_paths) != all(path is not None for path in promotion_paths):
        raise GroundedAuthorizationError("navigation promotion inputs must be supplied together")
    cross_entity_paths = (
    ...
    if any(path is not None for path in cross_entity_paths) != all(path is not None for path in cross_entity_paths):
        raise GroundedAuthorizationError("cross-entity promotion inputs must be supplied together")
    approval_bindings = semantic_approval_bindings or bindings
    ...
    semantic_corrections: dict[tuple[int, str, str], dict[str, Any]] = {}
    if all(path is not None for path in correction_paths):
        semantic_corrections = load_semantic_binding_corrections(
            packets=semantic_correction_packets,
            decisions=semantic_correction_decisions,
            manifest_path=semantic_correction_manifest,
            base_bindings=approval_bindings,
            structured_tables=structured_tables,
        )
        for key, correction in semantic_corrections.items():
            base_approval = semantic_approvals.get(key)
            if base_approval is None:
                raise GroundedAuthorizationError(
                    f"semantic correction lacks superseded human approval: {key}"
                )
            current_question = binding_rows.get(key[0])
            current_stage = next(
                (
                    stage
                    for stage in (current_question or {}).get("stages") or []
                    if _text(stage.get("stage_id")) == key[1]
                ),
                None,
            )
            current_operand = next(
                (
                    operand
                    for operand in (current_stage or {}).get("required_operands") or []
                    if _text(operand.get("role")) == key[2]
                ),
                None,
            )
            correction_receipt = _mapping(_mapping(current_operand).get("semantic_correction"))
            decision = _mapping(correction.get("decision"))
            if (
                not current_operand
                or _text(current_operand.get("concept_id")) != _text(decision.get("corrected_variable_id"))
                or correction_receipt.get("correction_decision_sha256")
                != decision.get("correction_decision_sha256")
                or correction_receipt.get("numeric_value_selected_by_reviewer") is not False
            ):
                raise GroundedAuthorizationError(
                    f"semantic correction was not deterministically materialized: {key}"
                )
            semantic_approvals[key] = effective_approval_from_correction(
                base_approval=base_approval,
                correction=correction,
            )
    navigation_promotions: dict[tuple[int, str, str], dict[str, Any]] = {}
    if all(path is not None for path in promotion_paths):
        navigation_promotions = load_navigation_binding_promotions(
            packets=navigation_promotion_packets,
            decisions=navigation_promotion_decisions,
            manifest_path=navigation_promotion_manifest,
            base_bindings=navigation_promotion_base_bindings,
            structured_tables=structured_tables,
            evidence_context=evidence_context,
        )
        for key, promotion in navigation_promotions.items():
            if key in semantic_approvals:
                raise GroundedAuthorizationError(
                    f"navigation promotion conflicts with an existing semantic approval: {key}"
                )
            current_question = binding_rows.get(key[0])
            current_stage = next(
                (
                    stage for stage in (current_question or {}).get("stages") or []
                    if _text(stage.get("stage_id")) == key[1]
                ),
                None,
            )
            current_operand = next(
                (
                    operand for operand in (current_stage or {}).get("required_operands") or []
                    if _text(operand.get("role")) == key[2]
                ),
                None,
            )
            receipt = _mapping(_mapping(current_operand).get("navigation_promotion"))
            decision = _mapping(promotion.get("decision"))
            if (
                not current_operand
                or _text(current_operand.get("binding_status")) != "binding_ready"
                or _text(current_operand.get("concept_id")) != _text(decision.get("approved_variable_id"))
                or receipt.get("promotion_decision_sha256")
                != decision.get("promotion_decision_sha256")
                or receipt.get("numeric_value_selected_by_reviewer") is not False
            ):
                raise GroundedAuthorizationError(
                    f"navigation promotion was not deterministically materialized: {key}"
                )
            semantic_approvals[key] = effective_approval_from_navigation_promotion(promotion)
    cross_entity_promotions: dict[int, dict[str, Any]] = {}
    if all(path is not None for path in cross_entity_paths):
        cross_entity_promotions = load_cross_entity_binding_promotions(
            packets=cross_entity_promotion_packets,
            decisions=cross_entity_promotion_decisions,
            manifest_path=cross_entity_promotion_manifest,
            base_bindings=cross_entity_promotion_base_bindings,
            structured_tables=structured_tables,
            evidence_context=evidence_context,
            repository_root=repository_root,
        )
        for question_id, promotion in cross_entity_promotions.items():
            approvals = effective_approvals_from_cross_entity_promotion(promotion)
            for key, approval in approvals.items():
                if key in semantic_approvals:
                    raise GroundedAuthorizationError(
                        f"cross-entity promotion conflicts with an existing semantic approval: {key}"
                    )
                current_question = binding_rows.get(question_id)
                current_stage = next(
                    (
                        stage for stage in (current_question or {}).get("stages") or []
                        if _text(stage.get("stage_id")) == key[1]
                ...
                receipt = _mapping(_mapping(current_operand).get("cross_entity_promotion"))
                decision = _mapping(promotion.get("decision"))
                if (
                    not current_operand
                    or _text(current_operand.get("binding_status")) != "binding_ready"
                    ...
                    raise GroundedAuthorizationError(
                        f"cross-entity promotion was not deterministically materialized: {key}"
                    )
                semantic_approvals[key] = approval
    ...
    readiness = {
        ...
        "chatgpt_semantic_correction_count": len(semantic_corrections),
        "chatgpt_navigation_promotion_count": len(navigation_promotions),
        "chatgpt_cross_entity_promotion_count": len(cross_entity_promotions),
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "semantic_approval_bindings": (
                {"path": str(approval_bindings), "sha256": sha256_file(approval_bindings)}
                if semantic_approval_bindings is not None
                else None
            ),
            "semantic_correction_packets": (
                {"path": str(semantic_correction_packets), "sha256": sha256_file(semantic_correction_packets)}
                if semantic_correction_packets is not None
                else None
            ),
            "semantic_correction_decisions": (
                {"path": str(semantic_correction_decisions), "sha256": sha256_file(semantic_correction_decisions)}
                if semantic_correction_decisions is not None
                else None
            ),
            "semantic_correction_manifest": (
                {"path": str(semantic_correction_manifest), "sha256": sha256_file(semantic_correction_manifest)}
                if semantic_correction_manifest is not None
                else None
            ),
            "navigation_promotion_base_bindings": (
                {
                    "path": str(navigation_promotion_base_bindings),
                    "sha256": sha256_file(navigation_promotion_base_bindings),
                }
                if navigation_promotion_base_bindings is not None
                else None
            ),
            "navigation_promotion_packets": (
                {
                    "path": str(navigation_promotion_packets),
                    "sha256": sha256_file(navigation_promotion_packets),
                }
                if navigation_promotion_packets is not None
                else None
            ),
            "navigation_promotion_decisions": (
                {
                    "path": str(navigation_promotion_decisions),
                    "sha256": sha256_file(navigation_promotion_decisions),
                }
                if navigation_promotion_decisions is not None
                else None
            ),
            "navigation_promotion_manifest": (
                {
                    "path": str(navigation_promotion_manifest),
                    "sha256": sha256_file(navigation_promotion_manifest),
                }
                if navigation_promotion_manifest is not None
                else None
            ),
            "cross_entity_promotion_base_bindings": (
                {
                    "path": str(cross_entity_promotion_base_bindings),
                    "sha256": sha256_file(cross_entity_promotion_base_bindings),
                }
                if cross_entity_promotion_base_bindings is not None
                else None
            ),
            "cross_entity_promotion_packets": (
                {
                    "path": str(cross_entity_promotion_packets),
                    "sha256": sha256_file(cross_entity_promotion_packets),
                }
                if cross_entity_promotion_packets is not None
                else None
            ),
            "cross_entity_promotion_decisions": (
                {
                    "path": str(cross_entity_promotion_decisions),
                    "sha256": sha256_file(cross_entity_promotion_decisions),
                }
                if cross_entity_promotion_decisions is not None
                else None
            ),
            "cross_entity_promotion_manifest": (
                {
                    "path": str(cross_entity_promotion_manifest),
                    "sha256": sha256_file(cross_entity_promotion_manifest),
                }
                if cross_entity_promotion_manifest is not None
                else None
            ),
