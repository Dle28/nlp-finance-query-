#!/usr/bin/env python3
"""Build blind source-replay intake packets for release-gate replay blockers.

Packets contain only immutable question/typed-plan contracts.  They omit
retrieval candidates, prior critic output, values, table IDs and cells, so a
reviewer must rediscover and validate source material independently.  This is
not a source replay result, a label, an answer, or a route/plan materializer.
"""
...
PROTOCOL = "independent_source_replay_intake_queue_v1"
...
LANE = "independent_source_replay"
SOURCE_CONTRACT = {
...
def validate_release_gate(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("Source-replay intake requires a blocked non-promotable release gate")
    return gate
...
def validate_remediation_queue(
    *, path: Path, manifest_path: Path, release_gate: Path, typed_plans_sha: str
    ...
    if not lane_ids:
        raise ValueError("Remediation queue contains no independent-source-replay lane")
    return lane_ids


def typed_plan_contract(plan: Mapping[str, Any]) -> dict[str, Any]:
...
def build(
    *, bundle_dir: Path, typed_plans: Path, release_gate: Path,
    ...
    """Write one blind intake record for each independent-source-replay blocker."""
    output_dir.mkdir(parents=True, exist_ok=True)
    queue_path = output_dir / "independent_source_replay_intake_queue_v1.jsonl"
    manifest_path = output_dir / "independent_source_replay_intake_queue_v1.manifest.json"
    if queue_path.exists() or manifest_path.exists():
        raise FileExistsError(f"Refusing to overwrite source-replay intake: {output_dir}")
    ...
    rows: list[dict[str, Any]] = []
    for question_id in sorted(lane_ids, key=lambda value: (str(plans[value].get("plan_fingerprint") or ""), value)):
        context = {
            "question": items[question_id].get("question"),
            "question_plan": items[question_id].get("question_plan"),
            "typed_plan_contract": typed_plan_contract(plans[question_id]),
        }
        rows.append(
            {
                "schema_version": 1,
                ...
                "review_instructions": [
                    "Do not inspect prior retrieval candidates, independent-critic output or any model/reviewer decision.",
                    "Rediscover source material independently, then verify exact entity, period, scope, unit and V2/V3 coordinates for every required operand.",
                    "Retain blocked when no single valid source binding can be established or exact sources conflict.",
                    "Do not record values, selected tables/cells, formulas, execution results, answers, labels or eligibility decisions in this intake.",
                ],
    ...
    if len({row["question_id"] for row in rows}) != len(rows) or {row["question_id"] for row in rows} != lane_ids:
        raise ValueError("Source-replay intake queue must cover its remediation lane exactly once")
    ...
    manifest = {
        "schema_version": 1,
        "protocol": PROTOCOL,
        "queue_status": "blind_source_review_intake",
