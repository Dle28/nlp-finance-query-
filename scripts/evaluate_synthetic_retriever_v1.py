#!/usr/bin/env python3
"""Evaluate base and fine-tuned retrievers on issuer-held-out synthetic rows.

This evaluator deliberately accepts only ``validation`` and ``test`` splits of
the execution-verified synthetic curriculum.  It never reads ViFinQA benchmark
questions and never promotes a model: it writes comparison evidence for a
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Repeat exactly as, for example, base=BAAI/bge-m3 and finetuned=/path/to/model.",
    )
...
def load_held_out_rows(
    path: Path, *, splits: Iterable[str], expected_tables_sha256: str
) -> dict[str, list[dict[str, Any]]]:
    selected = tuple(splits)
    if not selected or set(selected) - HELD_OUT_SPLITS:
        raise ValueError("Evaluation is restricted to issuer-held-out validation and test splits")
    ...
    for split, rows in rows_by_split.items():
        if not rows:
            raise ValueError(f"No validated held-out rows for split={split!r}")
...
def _rank_model(
    ...
    *,
    ...
    for start in range(0, len(rows), query_batch_size):
        ...
        scores = query_embeddings[start:stop] @ passage_embeddings.T
...
def main() -> None:
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise FileExistsError(f"Refusing to overwrite evaluation artifact: {args.output_dir}")
    ...
    rows_by_split = load_held_out_rows(
        args.curriculum, splits=args.splits, expected_tables_sha256=expected_tables_sha256
    )
    assets = {
    ...
    for label, reference in models.items():
        ...
        for split, rows in rows_by_split.items():
            metrics, row_results = summarise_rankings(rows, rankings, assets, ks=args.ks)
            _write_jsonl(args.output_dir / f"{label}_{split}_rankings.jsonl", row_results)
            split_metrics[split] = metrics
        model_results[label] = {"identity": _file_identity(reference), "splits": split_metrics}
    payload = {
