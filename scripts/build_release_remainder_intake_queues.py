#!/usr/bin/env python3
"""Build three blank, source-only intakes for remaining release blockers.

The release remediation queue is the sole authority for non-overlapping lane
membership.  These outputs are workload handoffs only: they contain immutable
question/typed-plan contracts, no prior candidates, source values/cells,
critic outputs, answers, labels, execution output or promotion permission.
"""
...
def validate_release_gate(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("Remaining-lane intake requires a blocked non-promotable release gate")
    return gate


def validate_remediation_queue(
    *, path: Path, manifest_path: Path, release_gate: Path, typed_sha: str
) -> dict[str, set[int]]:
    ...
    for question_id, row in rows.items():
        ...
        lane = row.get("remediation_lane")
        if lane in output:
            output[str(lane)].add(question_id)
    if any(not output[lane] for lane in LANES):
        raise ValueError("Remediation queue lacks a required remaining lane")
    if len(set.union(*output.values())) != sum(len(ids) for ids in output.values()):
        raise ValueError("Remaining remediation lanes must be disjoint")
    return output
...
def lane_metadata(lane: str) -> tuple[str, list[str]]:
    if lane == "deterministic_executor_compile":
        return (
            "blank_executor_contract_review",
            [
                "Do not compile an executor until exact source evidence is independently complete for every operand.",
                "Confirm operator semantics, arity, unit/scope guards, zero division and output type through tests.",
                "Do not record source values/cells, execution output, answer, label or eligibility here.",
            ],
        )
    if lane == "exact_source_conflict_adjudication":
        return (
            "blank_exact_source_conflict_review",
            [
                "Reopen source materials independently; do not inspect prior candidate values or critic decisions.",
                "Establish whether a single source binding exists under exact entity, period, scope and unit constraints; otherwise retain conflict.",
                "Do not record values, selected tables/cells, answer, label or eligibility here.",
            ],
        )
    if lane == "query_program_shadow_completion":
        return (
            "blank_query_program_shadow_review",
            [
                "Confirm the formula/typed operation contract and exact source prerequisites before compiling a shadow program.",
                "Keep the item blocked when program semantics or exact-source operands are incomplete.",
                "Do not record source values/cells, execution output, answer, label or eligibility here.",
            ],
        )
    raise ValueError(f"Unsupported remediation lane: {lane}")
...
def build(
    *, bundle_dir: Path, typed_plans: Path, release_gate: Path,
    ...
    """Write one blank, source-only handoff per remaining remediation lane."""
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = output_dir / "production_release_remainder_intakes_v1.manifest.json"
    if manifest_path.exists():
        raise FileExistsError(f"Refusing to overwrite remaining-lane intake manifest: {manifest_path}")
    ...
    lane_family_counts: dict[str, dict[str, int]] = {}
    for lane in LANES:
        intake_kind, instructions = lane_metadata(lane)
        rows: list[dict[str, Any]] = []
        for question_id in sorted(lane_ids[lane]):
            context = {
                "question": items[question_id].get("question"),
                "question_plan": items[question_id].get("question_plan"),
                "typed_plan_contract": typed_contract(plans[question_id]),
        ...
        if len({row["question_id"] for row in rows}) != len(rows):
            raise ValueError(f"{lane} intake contains duplicate question IDs")
        path = output_dir / f"{lane}_intake_v1.jsonl"
        if path.exists():
            raise FileExistsError(f"Refusing to overwrite {lane} intake: {path}")
        path.write_text("".join(json.dumps(row, ensure_ascii=False, sort_keys=True) + "\n" for row in rows), encoding="utf-8")
        outputs[lane] = {"path": str(path), "sha256": sha256_file(path)}
        lane_counts[lane] = len(rows)
        lane_family_counts[lane] = dict(
            sorted(Counter(str(row["review_context"]["typed_plan_contract"]["effective_family"]) for row in rows).items())
        )
    manifest = {
        "schema_version": 1,
        "protocol": PROTOCOL,
        "queue_status": "blank_source_review_intakes",
...
if __name__ == "__main__":
    ...
    print(json.dumps(build(**vars(args))["lane_counts"], ensure_ascii=False, sort_keys=True))
