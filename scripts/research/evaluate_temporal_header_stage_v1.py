#!/usr/bin/env python3
"""Evaluate one frozen temporal-header stage."""
...
from finance_query.research.temporal_header_eval import (
    evaluate_temporal_header_stage,
    load_jsonl,
)
...
def main() -> None:
    ...
    if args.stage == "untouched_evaluation":
        if args.pre_unseen_freeze is None or not args.pre_unseen_freeze.is_file():
            raise SystemExit("untouched temporal evaluation requires pre-unseen freeze")
        receipt = json.loads(args.pre_unseen_freeze.read_text(encoding="utf-8"))
        if (
            receipt.get("protocol") != "temporal_header_semantics_pre_unseen_freeze_v1"
            or receipt.get("untouched_evaluation_allowed") is not True
        ):
            raise SystemExit("invalid temporal pre-unseen receipt")
        validation = validate_temporal_pre_unseen(args.pre_unseen_freeze, REPO_ROOT)
        if validation["status"] != "PASS":
            raise SystemExit("temporal pre-unseen receipt validation failed")
    ...
    manifest = {
        "protocol": "temporal_header_semantics_evaluation_manifest_v1",
        "schema_version": 1,
        "inputs": {
