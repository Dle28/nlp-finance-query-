#!/usr/bin/env python3
"""Build candidate-only period repairs from rechecked V3 source titles."""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[2]
if str(REPO_ROOT / "src") not in sys.path:
    sys.path.insert(0, str(REPO_ROOT / "src"))

from finance_query.research.source_title_period_recheck import build_source_title_period_recheck


def main() -> None:
    parser = argparse.ArgumentParser()
    for name in (
        "triage",
        "base-period-packets",
        "base-period-manifest",
        "route-overlay",
        "route-overlay-manifest",
        "discovery-candidates",
        "structured-tables",
        "evidence-context",
        "evidence-context-manifest",
        "output-dir",
    ):
        parser.add_argument(f"--{name}", type=Path, required=True)
    parser.add_argument("--expected-question-count", type=int, default=1012)
    args = parser.parse_args()
    result = build_source_title_period_recheck(
        triage_path=args.triage,
        base_period_packets_path=args.base_period_packets,
        base_period_manifest_path=args.base_period_manifest,
        route_overlay_path=args.route_overlay,
        route_overlay_manifest_path=args.route_overlay_manifest,
        discovery_candidates_path=args.discovery_candidates,
        structured_tables_path=args.structured_tables,
        evidence_context_path=args.evidence_context,
        evidence_context_manifest_path=args.evidence_context_manifest,
        output_dir=args.output_dir,
        expected_question_count=args.expected_question_count,
    )
    print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
