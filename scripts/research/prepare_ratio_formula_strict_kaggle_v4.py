#!/usr/bin/env python3
"""Stage qualifier-aware strict exact-ratio v4 for Kaggle CPU."""
...
V4_RUNNER = ROOT / "scripts/research/run_ratio_formula_variant_v4.py"

RUNTIME_SLUG = "dungle2810/vifinqa-exact-ratio-strict-v4-20260830"
KERNEL_SLUG = "dungle2810/vifinqa-exact-ratio-strict-cpu-v4-20260830"
PROTOCOL = "vifinqa_exact_ratio_formula_v4"
WORK_NAME = "vifinqa_exact_ratio_strict_cpu_v4"
...
def _copy_runtime(runtime_dir: Path) -> None:
    ...
    new_archive = runtime_dir / "runtime_code.v4.tar"
    with tarfile.open(old_archive, mode="r") as source, tarfile.open(
        ...
        target.add(V4_RUNNER, arcname="scripts/research/run_ratio_formula_variant_v4.py")
...
def _treatment_notebook() -> dict[str, Any]:
    ...
    replacements = {
        "run_ratio_formula_variant_v1.py": "run_ratio_formula_variant_v4.py",
    ...
    if "run_ratio_formula_variant_v4.py" not in code:
        raise AssertionError("v4 runner was not wired into notebook")
    if PROTOCOL not in code:
        raise AssertionError("v4 protocol was not wired into notebook")
    for cell in notebook.get("cells", []):
        if cell.get("cell_type") == "code":
            cell["source"] = [line + "\n" for line in code.splitlines()]
        elif cell.get("cell_type") == "markdown":
            cell["source"] = [
                "# ViFinQA exact ratio qualifier-aware strict treatment (CPU)\n",
                "\n",
                "This treatment preserves total/provision row qualifiers and scope.\n",
            ]
...
def stage(output_root: Path) -> dict[str, str]:
    ...
    for path in (BASE_RUNTIME, V2_RUNNER, V4_RUNNER):
        if not path.exists():
    ...
    _write_json(
        ...
        {
            "title": "ViFinQA Exact Ratio Strict Runtime V4 20260830",
            "id": RUNTIME_SLUG,
            "licenses": [{"name": "other"}],
            "description": (
                "Immutable v1 canonical runtime plus qualifier-aware strict "
                "row-identity and fail-closed scope treatment adapter."
            ),
        },
    )
    ...
    _write_json(
        ...
        {
            "id": KERNEL_SLUG,
            "title": "ViFinQA Exact Ratio Strict CPU V4 20260830",
...
def main() -> None:
    output_root = ROOT / (
        "artifacts/kaggle_upload/"
        "dungle2810_vifinqa_exact_ratio_strict_cpu_v4_20260830"
    )
