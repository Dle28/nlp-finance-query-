...
from finance_query.research.proof_policy.active_learning_models import (  # noqa: E402
    MODEL_JOB_PROTOCOL,
...
from finance_query.research.proof_policy.evidence_closure import canonical_sha256, load_json, load_jsonl, sha256_file  # noqa: E402
from finance_query.research.llm.diagnostic_lane import (  # noqa: E402
    LLM_DIAGNOSTIC_BATCH_PROTOCOL,
)


def _request_output_name(job: Mapping[str, Any], role: str) -> str:
    protocol = job.get("protocol")
    status = job.get("status")
    if protocol == MODEL_JOB_PROTOCOL and status == "PREPARED_GPU_EXECUTION_NOT_RUN":
        if role != "open_source_model_proposer":
            raise ValueError("the canonical model job has one Qwen proposer route")
        return "proposer_requests"
    if (
        protocol == LLM_DIAGNOSTIC_BATCH_PROTOCOL
        and status == "PREPARED_PROPOSER_DIAGNOSTIC_NOT_RUN"
    ):
        if role != "open_source_model_proposer":
            raise ValueError("bounded diagnostic only supports the Qwen proposer")
        return "proposer_requests"
    raise ValueError("invalid active-learning or diagnostic model job")
...
def main() -> None:
    ...
    parser.add_argument("--role", choices=["open_source_model_proposer"], required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--limit", type=int)
    args = parser.parse_args()
    job = load_json(args.job_manifest.resolve())
    output_name = _request_output_name(job, args.role)
