#!/usr/bin/env python3
"""Stage the strict ratio v2 treatment for Kaggle CPU.

The completed v1 control is a valid same-builder control for this follow-up:
the new runtime contains the identical canonical builder and input files, plus
only the v2 adapter.  Reusing that immutable control avoids launching a third
full-corpus CPU job while keeping the builder/source-first fingerprints
comparable.
"""
...
RUNTIME_SLUG = "dungle2810/vifinqa-exact-ratio-strict-v2-20260830"
KERNEL_SLUG = "dungle2810/vifinqa-exact-ratio-strict-cpu-v2-20260830"
PROTOCOL = "vifinqa_exact_ratio_formula_v2"
WORK_NAME = "vifinqa_exact_ratio_strict_cpu_v2"
...
def _copy_runtime(runtime_dir: Path) -> None:
    ...
    new_archive = runtime_dir / "runtime_code.v2.tar"
...
def _treatment_notebook() -> dict[str, Any]:
    ...
    replacements = {
        "run_ratio_formula_variant_v1.py": "run_ratio_formula_variant_v2.py",
    ...
    if "run_ratio_formula_variant_v2.py" not in code:
        raise AssertionError("v2 runner was not wired into notebook")
    if PROTOCOL not in code:
        raise AssertionError("v2 protocol was not wired into notebook")
    for cell in notebook.get("cells", []):
        if cell.get("cell_type") == "code":
            cell["source"] = [line + "\n" for line in code.splitlines()]
        elif cell.get("cell_type") == "markdown":
            cell["source"] = [
                "# ViFinQA exact ratio strict row-identity treatment (CPU)\n",
                "\n",
                "This treatment adds hard row anchors and fail-closed explicit scope.\n",
            ]
...
def stage(output_root: Path) -> dict[str, str]:
    ...
    if not V2_RUNNER.exists():
        raise FileNotFoundError(V2_RUNNER)
    ...
    _write_json(
        ...
        {
            "title": "ViFinQA Exact Ratio Strict Runtime V2 20260830",
            "id": RUNTIME_SLUG,
            "licenses": [{"name": "other"}],
            "description": (
                "Immutable v1 canonical runtime plus strict row-identity and "
                "fail-closed scope treatment adapter."
            ),
        },
    )
    ...
    _write_json(
        ...
        {
            "id": KERNEL_SLUG,
            "title": "ViFinQA Exact Ratio Strict CPU V2 20260830",
...
def main() -> None:
    output_root = ROOT / (
        "artifacts/kaggle_upload/"
        "dungle2810_vifinqa_exact_ratio_strict_cpu_v2_20260830"
    )
