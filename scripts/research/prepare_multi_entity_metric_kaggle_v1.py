#!/usr/bin/env python3
"""Stage a CPU Kaggle dataset and kernel for the multi-entity metric ablation."""
...
VARIANT_RUNNER = ROOT / "scripts/research/run_multi_entity_metric_variant_v1.py"

RUNTIME_SLUG = "dungle2810/vifinqa-primary-multi-entity-metric-v1-20260830"
KERNEL_SLUG = "dungle2810/vifinqa-multi-entity-metric-cpu-v1-20260830"
...
WORK = Path('/kaggle/working/vifinqa_multi_entity_metric_cpu_v1')
...
if runtime_archive.exists():
    ...
    # Kaggle may expand an uploaded tar into a directory while creating the
    # dataset.  Accept both layouts so the input contract is stable across
    # CLI/server versions.
...
runner = RUNTIME_ROOT / 'scripts/research/run_multi_entity_metric_variant_v1.py'
...
print(run.stdout[-24000:])
if run.returncode:
    print(run.stderr[-24000:])
    raise RuntimeError(f'variant builder failed with exit code {run.returncode}')
if run.stderr:
    print(run.stderr[-12000:])

report = json.loads((output_dir / 'build_report.json').read_text())
print(json.dumps({
    'protocol': 'vifinqa_multi_entity_metric_plan_v1',
    ...
    'multi_entity_metric_variant': report.get('multi_entity_metric_variant'),
        ...
        "cells": [
            {
                ...
                "metadata": {},
                "source": [
                    "# ViFinQA multi-entity metric/plan ablation (CPU)\n",
                    "\n",
                    "This run changes only the simple multi-entity aggregation lane. Complex or abstained plans are not forced into an answer.\n",
                ],
            },
            {
                "cell_type": "code",
...
def stage(
    ...
    *,
    ...
    with tarfile.open(runtime_tar, mode="w") as archive:
        archive.add(BUILDER, arcname="scripts/e2e/build_competition_submission_v1.py", filter=_tar_filter)
        archive.add(VARIANT_RUNNER, arcname="scripts/research/run_multi_entity_metric_variant_v1.py", filter=_tar_filter)
        archive.add(ROOT / "src/finance_query", arcname="src/finance_query", filter=_tar_filter)

    _write_json(
        runtime_dir / "dataset-metadata.json",
        {
            "title": "ViFinQA Multi Entity CPU Runtime V1r3",
            "id": runtime_slug,
            "licenses": [{"name": "other"}],
            "description": (
                "CPU runtime for a controlled multi-entity aggregation/typed-plan ablation. "
                "Complex plans remain unforced; final cells are current structured-table Decimal replays."
            ),
        },
    )
    ...
    (kernel_dir / "main.ipynb").write_text(
        json.dumps(_notebook(runtime_slug, kernel_slug=kernel_slug), ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    _write_json(
        ...
        {
            "id": kernel_slug,
            "title": "ViFinQA Multi Entity Metric CPU V1r3 20260830",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=ROOT / "artifacts/kaggle_upload/dungle2810_vifinqa_multi_entity_metric_cpu_v1_20260830",
    )
    parser.add_argument("--runtime-slug", default=RUNTIME_SLUG)
    parser.add_argument("--kernel-slug", default=KERNEL_SLUG)
    args = parser.parse_args()
    print(json.dumps(stage(args.output_root, runtime_slug=args.runtime_slug, kernel_slug=args.kernel_slug), indent=2))
