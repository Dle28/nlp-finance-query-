...
queue = _module("build_formula_evidence_materialization_intake_queue", "build_formula_evidence_materialization_intake_queue.py")
mod = _module("validate_formula_evidence_materialization_consensus", "validate_formula_evidence_materialization_consensus.py")
...
def _plan(question: str) -> dict:
    operands = []
    for operand_id, year in (("x_new", 2023), ("x_old", 2022)):
        operands.append(
            {
    ...
    return {
        ...
        "route": "simple_temporal_comparison",
        "reason_codes": ["TYPED_PLAN_COMPLETE"],
        "entities": ["ABC"],
        "years": [2022, 2023],
        "scope": "consolidated",
...
def _proposal() -> dict:
    return {
        "plan_fingerprint": "b" * 64,
        "required_operand_ids": ["x_new", "x_old"],
        "operation_ast": {"op": "subtract", "args": ["x_new", "x_old"]},
...
def _fixture(tmp_path: Path) -> dict[str, Path]:
    ...
    tables = bundle / "tables.jsonl"; tables.write_text("{}\n", encoding="utf-8")
    item = {"id": 7, "question": "Chênh lệch doanh thu ABC 2023 và 2022?", "question_plan": {"family": "temporal_change"}}
    review_items = bundle / "review_items.jsonl"; _write_jsonl(review_items, [item])
    ...
    context = {"question": item["question"], "question_plan": item["question_plan"], "typed_plan_contract": queue.typed_contract(plan)}
    intake_row = {"schema_version": 1, "protocol": "formula_evidence_materialization_intake_queue_v1", "question_id": 7, "plan_fingerprint": plan["plan_fingerprint"], "immutable_review_context_sha256": review.canonical_sha256(context), "review_context": context, "review_decision_contract": {"decision": None, "decision_provenance": None, "reviewer_id": None, "reviewed_at": None, "source_coordinates_checked": None, "proposed_formula_evidence_contract": None, "materialization_allowed": False}, "materialization_allowed": False, "source_contract": review.SOURCE_CONTRACT}
    intake = tmp_path / "intake.jsonl"; _write_jsonl(intake, [intake_row])
    intake_manifest = tmp_path / "intake.manifest.json"; _write_json(intake_manifest, {"protocol": "formula_evidence_materialization_intake_queue_v1", "question_count": 1, "labels_prepopulated": False, "materialization_allowed": False, "source_contract": review.SOURCE_CONTRACT, "inputs": {"bundle_review_items": _record(review_items), "typed_plans": _record(typed), "typed_plans_manifest": _record(typed_manifest), "release_gate": _record(gate), "remediation_queue": _record(remediation), "remediation_manifest": _record(remediation_manifest)}, "outputs": {"queue": _record(intake)}})
    ...
    for slot, reviewer_id in (("reviewer_a", "human-a"), ("reviewer_b", "human-b")):
        ...
        label.update({"decision": "accept", "decision_provenance": "human_verified", "reviewer_id": reviewer_id, "reviewed_at": "2026-08-13T00:00:00Z", "source_coordinates_checked": coordinates, "source_coordinates_sha256": review.canonical_sha256({"source_coordinates_checked": coordinates}), "proposed_formula_evidence_contract": _proposal(), "notes": "Independently reopened raw source for a later exact EvidenceSet rebuild.", "is_blank_template": False})
...
def test_formula_materialization_consensus_requires_new_evidence_set(tmp_path: Path) -> None:
    result = mod.validate(**_fixture(tmp_path), output_dir=tmp_path / "out")
    assert result["counts"] == {"reconciled_review_count": 1, "agreed_accept_source_validated_count": 1, "operator_counts": {"subtract": 1}}
    row = json.loads((tmp_path / "out" / "formula_evidence_materialization_consensus_validation_v1.jsonl").read_text())
    assert row["formula_evidence_status"] == "not_materialized"
    assert row["validation_state"] == "formula_materialization_contract_source_validated_non_materializable"
    assert row["materialization_allowed"] is False


def test_formula_materialization_consensus_rejects_mutated_reconciliation(tmp_path: Path) -> None:
    ...
    rows[0]["consensus_proposal"]["evidence_set_disposition"] = "materialize_from_human_note"
