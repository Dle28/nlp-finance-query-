...
from finance_query.grounded_e2e import (
...
class GroundedE2ETests(unittest.TestCase):
    ...
    def test_run_creates_new_hash_bound_research_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("finance_query.grounded_e2e.build_exact_cell_bindings", self._fake_bindings), patch(
                "finance_query.grounded_e2e.materialize_numeric_cell_tokens", self._fake_cell_tokens
            ), patch(
                "finance_query.grounded_e2e.run_grounded_execution", self._fake_execution
            ), patch(
                "finance_query.grounded_e2e.materialize_authorization_replay", self._fake_authorization
            ):
    ...
    def test_replay_refuses_existing_output_or_changed_input(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("finance_query.grounded_e2e.build_exact_cell_bindings", mutating_bindings), patch(
                "finance_query.grounded_e2e.materialize_numeric_cell_tokens", self._fake_cell_tokens
            ), patch(
                "finance_query.grounded_e2e.run_grounded_execution", self._fake_execution
            ), patch(
